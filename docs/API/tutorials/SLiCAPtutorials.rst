...
Examples use `KiCad version 9 <https://www.kicad.org/>`_ as schematic capture program. Please check if you have KiCad 9 or later installed before running these examples. If not, please upgrade or install KiCAD and configure its command: in the SLiCAP GUI via :menuselection:`File --> Configure SLiCAP…`, or manually in the **[commands]** section of the ``SLiCAP.ini`` file in your home directory (see `Installation <../userguide/install.html>`__).

Some Examples use `LTspice <https://www.analog.com/en/resources/design-tools-and-calculators/ltspice-simulator.html>`_ as schematic capture program. Please check if you have it installed before running these examples. If not, please install it and configure its command in the same way.
